...
layui.define(['layer','form','laytpl','ht_ajax','ht_config'], function (exports) {
    ...
    var baseUrl = config.ruleServicePath.basePath;
    /*//条件体
    var conditionInfo = {
        //条件拼接
        conditionExpression: '',
    ...
    var myUtil = {
        ...
        business:{
            ...
            init_url:baseUrl+'businessSystem/getAll',
            init_html: function () {
    ...
    var sceneUtil = {
        ...
        addCon:function (t) {
            ...
            a.removeClass("actionEntity");
            a.removeClass("conditionSZ");
            //优化，仅仅设置条件项
        ...
        entitySelectInit:function(businessId){
            ...
            $.get(baseUrl+"entityInfo/getEntitysAll",{"businessId":businessId},function(data){
        ...
        actionSelectInit:function(businessId){
            ...
            $.get(baseUrl+"actionInfo/getAll",{"businessId":businessId},function(data){
                var result = {list : data.data
                }
        ...
        actionParam:{
            ...
            paramVaEntityBack:function (a_obj) {
                ...
                $(a_obj).submenu({
                    ...
                    callback: function (obj, value) {
                        ...
                        //设置变量值
                        var itemId = $(a_obj).attr("data-itemId");
                        if( itemId == '' || itemId == undefined  ){
                            $(a_obj).attr("data-itemId",value);
                        }else{
                            $(a_obj).attr("data-itemId",itemId+","+value);
                        }
            ...
            setParamsHtmls:function(actionId,li){
                ...
                $(li).find(".param .actionVal").each(function(){
                    var actionValA = $(this);
                    sceneUtil.bindMoreInput(actionValA);
                });
            }
         ...
         * @param that
         */
        bindMoreInput:function (that) {

            $(that).submenu({
                data: [{value:1,text:'输入值'},{value:2,text:'选择变量'},{value:3,text:'四则运算公式'}],type:1,
                callback: function (obj, value) {
                    ...
                    if(value == 1){
                        ...
                        $(that).unbind("click");
                        //加载新的
                        sceneUtil.bandOneValInit( $(that));
                        $(that).addClass("actionVal");
                        ...
                        $(that).attr("data-value","");
                        $(that).text("请选择变量");
                        $(that).removeClass("conditionSZ");
                        $(that).addClass("actionEntity");
                        sceneUtil.actionParam.paramVaEntityBack(that);
                    }else if(value == 3){
                        $(that).attr("data-value","");
                        $(that).text("请输入四则运算公式");
                        $(that).removeClass("actionEntity");
                        $(that).addClass("conditionSZ");
                        $(that).calculator();
                    }
                    // console.log(obj, value);
                }
        ...
        addTtemsBank:function (value) {
            ...
            items.forEach(function (t) {
                ...
                for (var i = 0; i < sceneUtil.data.itemsBank.length;i++){
                    if(t == sceneUtil.data.itemsBank[i]){
                        flag = false;
                        break;
                    }
                }
                if(flag){
                    sceneUtil.data.itemsBank.push(t);
                }
        ...
        gradeInit: function() {
            ...
            $(".actionVal").each(function(){
                sceneUtil.bindMoreInput(this);
            });
        ...
        gradeTrInit: function(tr) {
            ...
            // sceneUtil.bandOneValInit( $(tr).find('.actionVal'));
            //多种输入方式
            $(tr).find('.actionVal').each(function(){
                sceneUtil.bindMoreInput(this);
            });
            //条件绑定
            sceneUtil.bandSelectValInit_condition($(tr).find(".con"),sceneUtil.data.condition,"选择条件");
        ...
        getGradeRuleData: function () {
            ...
            $("#table>tbody>tr").each(function () {
                ...
                $(conTd).find("ul li").each(function(i,e){
                    ...
                    //变量类型的哦
                    if($(e).find("a.val").hasClass("actionEntity")){
                        val = '#'+$(e).find("a.val").attr("data-key")+'#';
                    }
                    //变量类型的哦
                    if($(e).find("a.val").hasClass("conditionSZ")){
                        val = '@'+$(e).find("a.val").attr("data-value")+'@';
                    }
                ...
                var actionA = $(actionTd).find("a.actionVal");
                //转化
                if($(actionA).hasClass("conditionSZ")){
                    actionVal = '@'+$(actionA).attr("data-value")+'@';
                }
                ...
                actionParamValsList.push(actionValInfo);
                actionParamValsList.push(actionValQz);

                actionInfos.push(actionParamValsList);
            ...
            $("[data-itemId]").each(function(){
                sceneUtil.addTtemsBank($(this).attr("data-itemId"));
            });
