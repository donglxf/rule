...
ModelVerification.prototype = {
     ...
     * @param data
     */
     setFromValues:function(el, data) {
        for (var p in data) {
    ...
    initParam: function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            html += "<tbody><tr></tbody><td>" + data[i].itemName + "</td>";
    ...
    initOneValible: function (valible) {
        var tmpValue = valible.value;
        var html = "";
        //拼接每个input的name属性组成,这里仅仅需要U_user_acceptMoney中的最后一个acceptMoney
        var name =  "specialItemMap['"+valible.itemIdentify+"']";

        switch (valible.type) {
            case "select":
                html = this.initSelect(valible.itemName, valible.optionData);
                break;
            case "date":
                html = this.initDate(valible.itemName);
                break;
            case "time":
                html = this.initTime(valible.itemName);
                break;
            case "Double":
    ...
    initSence: function (senceData) {
        var senceName = senceData.entityName;
        var valiableData = senceData.items;
  ...
  /*
        ...
        html += '<div class="layui-form-item">\n' +
            ...
            '<input type="text" name="groupName" required  lay-verify="required" placeholder="请输入标题" autocomplete="off" class="layui-input" value=""+groupName>\n' +
            '</div>\n' +
            '</div>';
        html += '<div class="layui-form-item">\n' +
            '<label class="layui-form-label">业务线</label>\n' +
            '<div class="layui-input-block">\n' +
            '<input type="text" name="businessId" required  lay-verify="required" placeholder="请输入标题" autocomplete="off" class="layui-input">\n' +
            '</div>\n' +
            '</div>';*/

        //遍历生成特殊属性代码
        for (var i = 0; i < data.length; i++) {
            html += this.initSence(data[i]);
        }
        return html;
    }
}
...
ModelAutoVerification.prototype = {
    ...
    initLabel: function (name) {
        ...
        return '<div class="layui-input-inline" style="margin: 0px" >' +
            '<input type="text" value=' + name + ' class="layui-input" disabled="disabled" style="width: auto"/>' +
            '</div>';
    },
    /**
     * 初始化普通输入框
    ...
    initOneValible: function (valible) {
        ...
        var itemIdentify = valible.itemIdentify;
        var bindTable = valible.bindTable;
        var bindColumn = valible.bindColumn;
        return this.initInput(senceVersionId + '_' + itemIdentify, bindTable, bindColumn);
    },
    ...
    initModel: function (data) {
        ...
        html += ' <div class="layui-form-item" >\n' +
            ...
            '    </div>' +
            '<button type="reset" class="layui-btn layui-btn-primary">重 置</button>' +
            '<button class="layui-btn"  lay-submit="" lay-filter="save_auto" id="save_auto">保 99999存</button>\n' +
            '</div>';
